...
BridgeSim::BridgeSim(int bridge_length, int bridge_width,
    int detonation_height, int detonation_x, int detonation_y, int thread_Amount, int trial_Amount)
    ...
    trial_Amount(trial_Amount)
{
    ...
    setup(); //TODO try catch for bad allocation
}
...
void BridgeSim::preCompute() {
    ...
    computeAngleFromDetenationToTile(bridge_tile_distance_from_detonation_array, blast_to_tile_theta_array);

    //This should implement the third equation
    computeBasicPeakPressureForTiles(bridge_tile_distance_from_detonation_array, blast_tile_basic_peak_pressure);

    calculateImpulseValue(bridge_tile_distance_from_detonation_array, impulse_value_per_tile);

    calculatePeakPressuePerTile(bridge_tile_distance_from_detonation_array, blast_tile_basic_peak_pressure, 
        impulse_value_per_tile, peak_pressure_per_tile);

    // in milliseconds
    calculateTimeOfArrivalPerTile(bridge_tile_distance_from_detonation_array, time_of_arrival_per_tile);

    // in milliseconds
    calculateLoadDurationPerTile(bridge_tile_distance_from_detonation_array, time_of_arrival_per_tile);
}
...
void BridgeSim::computeDistanceBlastDetonationToTiles(std::unique_ptr<float[]>& arr) {
...
void BridgeSim::computeGroundDistanceDetonationToTiles(std::unique_ptr<float[]>& arr) {
    int Width = bridge_Width_Inches;
...
void BridgeSim::computeAngleFromDetenationToTile(const std::unique_ptr<float[]>& distanceArr, std::unique_ptr<float[]>& angleArr) {
...
void BridgeSim::computeBasicPeakPressureForTiles(const std::unique_ptr<float[]>& distanceArr, std::unique_ptr<float[]>& arr) {
...
void BridgeSim::calculateLoadDurationPerTile(const std::unique_ptr<float[]>& distanceArr, std::unique_ptr<float[]>& arr) {
    const float Charge_Weight_Pounds = 1000.0f;
...
void BridgeSim::calculateImpulseValue(const std::unique_ptr<float[]>& distanceArr, std::unique_ptr<float[]>& arr) {
...
void BridgeSim::calculateTimeOfArrivalPerTile(const std::unique_ptr<float[]>& distanceArr, std::unique_ptr<float[]>& arr) {
    const float Charge_Weight_Pounds = 1000.0f;
